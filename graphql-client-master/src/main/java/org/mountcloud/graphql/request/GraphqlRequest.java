...
public abstract class GraphqlRequest {
    ...
    public RequestParameter addParameter(String key, Object val){
    	RequestParameter requestParam =  getRequestParameter().addParameter(key,val);    
    	requestParameters.add(requestParam);
    	return requestParam;
    }
    
    public GraphqlRequest addRequestParameter(RequestParameter... reqParam){
        if(reqParam!=null&&reqParam.length>0){
            for(RequestParameter ra : reqParam){
            	requestParameters.add(ra);
            }
